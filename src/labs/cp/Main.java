    ...
    extends Object
{
                             ...
                             Namespace ns)      // namespace of command line arguments
    {
        ...
        for(int gameIdx = 0; gameIdx < numTrainingGames; ++gameIdx)
        {
            ...
            while (!isDone) {
                try {
                    // Get Q-values for the current state and select an action
                    Matrix qValues = qFunction.forward(state);
                    int action = argmax(qValues);

                    // Perform the selected action and observe the next state, reward, and termination flag
                    ...
                    if(isDone){
                         rb.addSample(nextState, 0, null);                   
                    }
                    // Update the current state
                    state = nextState;
                } catch (Exception e) {
                    System.err.println("Error during training game " + gameIdx + ": " + e.getMessage());
                    e.printStackTrace();
                    isDone = true; // Exit the loop on failure
                }
            }
        }
    }
